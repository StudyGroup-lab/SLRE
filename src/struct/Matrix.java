...
public class Matrix {
	...
	public void initializeUnif(double l, double h) {
		Random rd = new Random(123);
		for (int i = 0; i < iNumberOfRows; i++) {
			for (int j = 0; j < iNumberOfColumns; j++) {
				double dValue = rd.nextDouble();
				pData[i][j] = (h-l) * dValue + l;
			}
		}
	}
	...
	public void initializeGaussian(double dVar) {
		Random rd = new Random(123);
		for (int i = 0; i < iNumberOfRows; i++) {
			for (int j = 0; j < iNumberOfColumns; j++) {
				double dValue = Math.sqrt(dVar) * rd.nextGaussian();
				pData[i][j] = dValue;
			}
		}
	}
	...
	public void truncate(double dLower) {
		for (int i = 0; i < iNumberOfRows; i++) {
			for (int j = 0; j < iNumberOfColumns; j++) {
				double dValue = pData[i][j];
				if (pData[i][j] < dLower) {
	...
	public void projectRelu(double dLower, double dUpper) {
		for (int i = 0; i < iNumberOfRows; i++) {
			for (int j = 0; j < iNumberOfColumns; j++) {
				double dValue = pData[i][j];
				if (pData[i][j] < dLower) {
	...
	public void truncate_row(double dLower, int i) {
		for (int j = 0; j < iNumberOfColumns; j++) {
			double dValue = pData[i][j];
			if (pData[i][j] < dLower) {
				dValue = dLower;
